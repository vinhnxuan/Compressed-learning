...
def get_psnr(input_, logits):
    ...
    PIXEL_MAX = tf.reduce_max(input_)
...
x0,residual_layers,residual_vectors,h_fc02, y2 = model.setup_model_autofocus_phase(x, keep_prob, keep_prob2, phase_errors,nums)

#c_est=tf.reduce_max(coherence_tensor(W_fc01))
h_fc02_vector= tf.reshape(h_fc02,[-1,image_size*image_size])

#_loss_2= residual_loss(x_vector,residual_vectors,nums)+ get_mmse(x_vector,h_fc02_vector)
#_psnr = get_psnr(x_vector,residual_vectors[nums-1])

_loss_2 = get_mmse(phase_errors, y2)
_psnr =-get_mmse(phase_errors, y2)
  ...
  for ii in range(1):
      ...
      for epoch in range(max_epoch):
            ...
            for step in range(num_tr_batch):
                ...
                noise_data=np.random.normal(-1.0,1.0,(batch_size,image_size)).astype(np.float32)
            ...
            noise_data=np.random.normal(-1.0,1.0,(2425,image_size)).astype(np.float32)

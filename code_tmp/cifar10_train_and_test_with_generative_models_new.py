...
# generative models
import reconstruction_models as g_model
...
noise=tf.placeholder(tf.float32, [None,  _IMAGE_SIZE *  _IMAGE_SIZE ], name='noise')
...
y_est, MM, var_list_reconstruction = g_model.setup_model_dr2net(tf.matmul(z+noise,W_01),_IMAGE_SIZE * _IMAGE_SIZE)
...
loss_1= get_mmse(z,y_est)#+ 0.001*MM
...
def predict_test(W):
    ...
    zero_noise_data=np.zeros((10000,_IMAGE_SIZE*_IMAGE_SIZE)).astype(np.float32)
    
    noise_data=np.random.normal(0.0,0.1,(10000,_IMAGE_SIZE*_IMAGE_SIZE)).astype(np.float32)
    ...
    for j in range(1):
        ...
        for i in range(10000):
            ...
            for k in range(epoch_size):
                ...
                noise_data=np.zeros((_BATCH_SIZE,_IMAGE_SIZE*_IMAGE_SIZE)).astype(np.float32)
                sigma = np.random.rand(1)*0.1;
                
                noise_data=np.random.normal(0.0,sigma,(_BATCH_SIZE,_IMAGE_SIZE*_IMAGE_SIZE)).astype(np.float32)
...
def train_classification(acc_max,psnr_max, psnr_max_noise):
    ...
    for i in range(5000):
        ...
        for k in range(epoch_size):
            ...
            #noise_data=np.zeros((_BATCH_SIZE,_IMAGE_SIZE*_IMAGE_SIZE)).astype(np.float32)
            #noise_data=np.random.normal(0.0,0.1,(_BATCH_SIZE,_IMAGE_SIZE*_IMAGE_SIZE)).astype(np.float32)
            sigma = np.random.rand(1)*0.1;

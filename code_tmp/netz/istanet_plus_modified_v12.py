...
def conv2d_inv(x, W):
  w1= tf.shape(x)[0]
  return tf.nn.conv2d_transpose(x, W, strides=[1, 1, 1, 1],padding='SAME',output_shape=[w1,32,32,1])
...
def deep_residual_ista_mix(data,n_nodes,keep_prob):
   ...
   for i in range(n_nodes):
        ...
        W_conv5[i] = weight_variable([1, 1, 64, 57])
        b_conv5[i] = bias_variable([57])
        
        h_conv5[i] = tf.nn.relu(conv2d(h_conv4[i], W_conv5[i]) + b_conv5[i])
        
        W_conv6[i] = weight_variable([1, 1, 1, 57])
        b_conv6[i] = bias_variable([1])
        
        #if i%2==0 :
        
        h_conv6[i] = (conv2d_inv(h_conv5[i], W_conv6[i]) + b_conv6[i])+ x_im
        #else:
...
#W_fc03 = weight_variable([num_measurement, 700])
#b_fc03 = bias_variable([700])
#
#W_fc03_drop=tf.nn.dropout(W_fc03, keep_prob)
#
#
#h_fc03 = (tf.matmul(h_fc01_noisy, W_fc03_drop)+b_fc03)

#W_fc04 = weight_variable([500, num_measurement])
#b_fc04 = bias_variable([num_measurement])
#
...
#W_fc03 = weight_variable([num_measurement, 1024-num_measurement])
#b_fc03 = bias_variable([1024-num_measurement])
##
#h_fc03 = tf.nn.tanh(tf.matmul(h_fc01_noisy, W_fc03)+b_fc03)
##
#W_fc04 = weight_variable([1024-num_measurement, 1024])
#b_fc04 = bias_variable([1024])
##
#h_fc04 = (tf.matmul(h_fc03, W_fc04)+b_fc04)
##
#h_fc02=h_fc02_base+ h_fc04
...
#print(tf.shape(residual_vectors))
...
MM2= tf.reduce_mean(tf.abs(W_fc02))#tf.reduce_mean(tf.abs(W_conv1))+tf.reduce_mean(tf.abs(W_conv2))+tf.reduce_mean(tf.abs(W_conv3))+tf.reduce_mean(tf.abs(W_conv4))+tf.reduce_mean(tf.abs(W_conv5))+tf.reduce_mean(tf.abs(W_conv6))
...
ratio=0.002
_loss_2= residual_loss(x,residual_vectors,nums)+ get_mmse(x,h_fc02 )

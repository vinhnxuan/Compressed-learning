...
def conv7d(x, W):
  return tf.nn.conv2d(x, W, strides=[1, sliding_step, sliding_step, 1],padding='VALID')
...
x_1 = tf.reshape(x, [-1, 32, 32, 1])



W_conv01 = weight_variable([conv_win, conv_win, 1, layer_num])
b_conv01 = bias_variable([layer_num])

h_conv01 = (conv7d(x_1, W_conv01) + b_conv01)


W_conv03 = weight_variable([1, 1, layer_num, 300])
b_conv03 = bias_variable([300])

h_conv03 = tf.nn.relu(conv2d(h_conv01, W_conv03) + b_conv03)

W_conv02 = weight_variable([1, 1, 300, conv_win*conv_win])
b_conv02 = bias_variable([conv_win*conv_win])

h_conv02 = (conv2d(h_conv03, W_conv02) + b_conv02)


h_trans=tf.transpose(h_conv02,[1,2,0,3])


image_all=0

for i in range(sliding_win):
    count=0
    for j in range(sliding_win):
#        indices = [[i,j]]
#        h_tmp= tf.gather_nd(h_trans, indices)
#        h_image = tf.reshape(h_tmp, [-1, conv_win,conv_win])
#        if count ==0:
#            image_x=h_image
#        else:
#            image_x=tf.concat([image_x,h_image],2)
#            
#        count=count+1
#    if i ==0:
#        image_all=image_x
#    else:
#        image_all=tf.concat([image_all,image_x],1)
        i_start=i*sliding_step
        i_end = 32-i*sliding_step-conv_win
        
        j_start=j*sliding_step
        j_end = 32-j*sliding_step-conv_win
        
        
        paddings=[[0,0],[i_start,i_end],[j_start,j_end]]
        indices = [[i,j]]
        h_tmp= tf.gather_nd(h_trans, indices)
        h_image = tf.reshape(h_tmp, [-1, conv_win,conv_win])
        
        h_padding=tf.pad(h_image,paddings)
    
        image_all=image_all+h_padding
                
image_all=tf.nn.sigmoid(image_all)    
        
        
x_image = tf.reshape(image_all, [-1, 32, 32, 1])
...
h_conv3 = tf.nn.tanh(conv2d(h_conv2_norm, W_conv3) + b_conv3)
...
y = tf.nn.tanh(conv2d(h_conv5_norm, W_conv6) + b_conv6) + h_conv3_add
...
MM= tf.reduce_mean(tf.abs(W_conv01))+ tf.reduce_mean(tf.abs(W_conv02))+ tf.reduce_mean(tf.abs(W_conv03))

cos=_loss+ 0.001*MM

# Training algorithm
optimizer_1 = tf.train.AdamOptimizer(learning).minimize(cos)
optimizer_2 = tf.train.Avinh2Optimizer(learning).minimize(cos)

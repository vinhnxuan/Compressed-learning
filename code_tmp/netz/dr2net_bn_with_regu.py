...
with tf.Session() as sess:
  ...
  for epoch in range(max_epoch):
    ...
    loss=0
    if (epoch-ind_max >500) or checking:
    ...
    #batch_xs, batch_ys = mnist.train.next_batch(batch_size)
        _, c, W, reg = sess.run([optimizer, _loss, W_fc01, MM], feed_dict={x: trX[kk[start:end]], y_: trY[kk[start:end]], learning:learning_rate, noise:noise_data})
...
#        noise_data[kk,]=noise_data[kk,]*np.sqrt(ra)
    ...
    #    start = i * batch_size
    #    end = start + batch_size
    if (epoch-ind_max >500):
        break
        #acc=0
    val_acc += acc
    print(epoch, val_acc, loss,loss_reg, ind_max, acc_max)

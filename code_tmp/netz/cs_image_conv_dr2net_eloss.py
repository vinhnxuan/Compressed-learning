...
W_conv01 = weight_variable([3, 3, 1, 64])
b_conv01 = bias_variable([64])

h_conv01 = tf.nn.relu(conv2d(x_image, W_conv01) + b_conv01)

W_conv02 = weight_variable([3, 3, 64, 64])
b_conv02 = bias_variable([64])

h_conv02 = tf.nn.relu(conv2d(h_conv01, W_conv02) + b_conv02)

W_conv03 = weight_variable([3, 3, 64, 1])
b_conv03 = bias_variable([1])

h_conv03 = tf.nn.sigmoid(conv2d(h_conv02, W_conv03) + b_conv03)

h_conv03_vector=tf.reshape(h_conv03, [-1, 32*32])

# Convolutional layer 2
W_conv1 = weight_variable([11, 11, 1, 64])
b_conv1 = bias_variable([64])

h_conv1 = tf.nn.relu(conv2d(h_conv03, W_conv1) + b_conv1)
...
_loss2 = get_mmse(x,h_conv03_vector) + get_mmse(x,h_conv3_add_vector)


_loss = get_mmse(x,y_vector) +0.1* _loss2
...
with tf.Session() as sess:
  ...
  for epoch in range(max_epoch):
    ...
    #    end = start + batch_size
        ...
        #acc=0
    val_acc += acc
    print(epoch, val_acc, loss,loss_reg, loss_reg2, loss_reg3, ind_max, acc_max, coh)

...
W_fc02 = weight_variable([num_measurement, 500])
b_fc02 = bias_variable([500])

h_fc02 = tf.nn.relu(tf.matmul(h_fc01_noisy, W_fc02)+b_fc02)


W_fc04 = weight_variable([500, 500])
b_fc04 = bias_variable([500])

h_fc04 = tf.nn.relu(tf.matmul(h_fc02, W_fc04)+b_fc04)


W_fc03 = weight_variable([500,1024])
b_fc03 = bias_variable([1024])

h_fc03 = tf.nn.relu(tf.matmul(h_fc04, W_fc03)+b_fc03)

#keep_prob  = tf.placeholder(tf.float32)
#h_fc03_drop = tf.nn.dropout(h_fc03, keep_prob)

#W_fc04 = weight_variable([500, 1024])
#b_fc04 = bias_variable([1024])
#
#h_fc04 = tf.nn.relu(tf.matmul(h_fc02, W_fc04)+b_fc04)


x_image = tf.reshape(h_fc03, [-1, 32, 32, 1])
...
MM= tf.reduce_mean(tf.abs(W_fc02))+ tf.reduce_mean(tf.abs(W_fc03)) + tf.reduce_mean(tf.abs(W_fc01)) + tf.reduce_mean(tf.abs(W_fc04))
#MM = tf.reduce_mean(tf.abs(W_fc01))+ tf.reduce_mean(tf.abs(W_fc03))
ratio=0.002/4
...
objective= _loss + ratio*MM

# Training algorithm
optimizer_1 = tf.train.AdamOptimizer(learning).minimize(objective)
optimizer_2 = tf.train.Avinh2Optimizer(learning).minimize(objective)
...
with tf.Session() as sess:
  ...
  for epoch in range(max_epoch):
    ...
    #batch_xs, batch_ys = mnist.train.next_batch(batch_size)
        _, c , W = sess.run([optimizer, _loss, W_fc01], feed_dict={x: trX[kk[start:end]], y_: trY[kk[start:end]], learning:learning_rate,noise:noise_data})
    ...
    acc = sess.run(_psnr, feed_dict={x: valX, y_: valY, noise:noise_data})
        #acc=0
    val_acc += acc

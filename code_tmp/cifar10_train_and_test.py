    ...
    for j in range(1):
        ...
        for i in range(iterations):
            ...
            if (i_global % epoch_size == 0) or (i == num_iterations - 1):
                mutual_coherence,_loss, batch_acc, W_matrix = sess.run([m_coherence,loss, accuracy, W_01], feed_dict={x: batch_xs, y: batch_ys,keep_prob:1.0})
            ...
            if (i_global % epoch_size == 0) or (i == num_iterations - 1):
                ...
                if acc_max==0 or acc_max<=acc:
                    ...
                    tr_acc=batch_acc
                    w_opt = W_matrix
                    sci.savemat('tmp/W_matrix_'+str(num_measurement)+'.mat', {'w':w_opt})

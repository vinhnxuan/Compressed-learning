...
cs_matrix = np.load("sar_cs_mat_ratio_4.dat")
...
x0,residual_layers,residual_vectors,h_fc02= model.setup_model_autofocus_cs(x, keep_prob, keep_prob2, phase_errors, speck_errors,nums, b_size)
...
_loss_2= residual_loss(x_vector,residual_vectors,nums)+ 2e-4*loss_tv#+0.1*_loss_3#+ get_mmse(x_vector,h_fc02_vector)
...
def test_on_one_image():
    img = read_image_test()
    img_num=np.shape(img)[0]
    test_Y=np.zeros((img_num,10),dtype=np.int32)
    noise_data=np.random.normal(0.0,noise_var,(img_num,image_size)).astype(np.float32)
    #noise_data_speckling=np.random.gamma(100.0,1.0,(batch_size,image_size,image_size)).astype(np.float32)/100.
    batch_cs_matrix = np.reshape(np.tile(cs_matrix,(img_num,1)),[-1,image_size,image_size])
    noise_data_speckling=np.ones([img_num,image_size,image_size])
    #noise_da
    acc,img = sess.run([_psnr,residual_layers[nums-1]], feed_dict={x: img, y_: test_Y, keep_prob: 1.0, keep_prob2: 1.0, phase_errors:noise_data, speck_errors:noise_data_speckling, b_size:batch_cs_matrix})
...
file_saved="/tmp/saver/model_"+str(noise_var)+".ckpt"
  ...
  for ii in range(1):
      ...
      for epoch in range(max_epoch):
            ...
            #batch_xs, batch_ys = mnist.train.next_batch(batch_size)
                _, c, ltv, h_m = sess.run([optimizer,_loss_2,loss_tv, h_fc02], feed_dict={x: trX[kk[start:end]], y_: trY[kk[start:end]], keep_prob: keep_prob_value_1, keep_prob2: keep_prob_value_2, learning:learning_rate, phase_errors:noise_data_autofocus,speck_errors:noise_data_speckling, b_size:batch_cs_matrix})
            ...
            for trial in range(1):
                for step in range(num_val_batch):
                    ...
                    val_acc += float(acc)*np.shape(test_X)[0]
                    ssim_acc += float(get_ssim(test_X,img))*np.shape(test_X)[0]
            
            
                
            val_acc =val_acc/(1.*np.shape(valX)[0])
            ssim_acc =ssim_acc/(1.*np.shape(valX)[0])
            ...
            if epoch ==0 or acc_max<=val_acc:
                ...
                saver.save(sess,file_saved)
                #coherence_max=mutual_coherence
            if epoch%10==0:
                  plt.imshow(np.reshape(img0[0],[image_size,image_size]))
                  plt.gray()
                  plt.show()
                  
                  
                  
                  tst_size= np.shape(teX)[0]
                  noise_data_speckling=np.random.gamma(100.0,1.0,(tst_size,image_size,image_size)).astype(np.float32)/100.
                  noise_data=np.random.normal(0.0,noise_var,(tst_size,image_size)).astype(np.float32)
                    #noise_data=np.ones((end_size,image_size,image_size)).astype(np.float32)
                    #noise_data=np.random.gamma(1.0,1.0,(end_size,image_size,image_size)).astype(np.float32)
                  noise_data_speckling=np.ones([tst_size,image_size,image_size])
                  batch_cs_matrix = np.reshape(np.tile(cs_matrix,(tst_size,1)),[-1,image_size,image_size])
        
                  acc,img2 = sess.run([_psnr,residual_layers[nums-1]], feed_dict={x: teX, y_: teY, keep_prob: 1.0, keep_prob2: 1.0, phase_errors:noise_data, speck_errors:noise_data_speckling, b_size:batch_cs_matrix})
                  
                  acc00,img00=test_on_one_image()
                  
                  print("Accuracy on MSTAR ", acc, acc00)
                  
                  plt.imshow(valX[0])
                  plt.gray()
                  plt.show()
                  
                  plt.imshow(np.reshape(img00[2],[image_size,image_size]))
                  plt.gray()
                  plt.show()
                  
                  img_org=read_image_test()
                  
                  plt.imshow(np.reshape(img_org[2],[image_size,image_size]))
                  plt.gray()
                  plt.show()
                  
                  plt.imshow(np.reshape(img00[5],[image_size,image_size]))
                  plt.gray()
                  plt.show()
                  
                  img_org=read_image_test()
                  
                  plt.imshow(np.reshape(img_org[5],[image_size,image_size]))
                  plt.gray()
                  plt.show()

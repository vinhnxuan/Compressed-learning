...
def residual_block(
    x, in_filter, out_filter, stride, activate_before_residual=False, activate_sigmoid=False,activate_relu=True,block_num=2):
  ...
  with tf.variable_scope('sub2'):
    ...
    if activate_sigmoid and block_num==1:
        block_x = tf.nn.sigmoid(block_x)
    else:
        if activate_relu or block_num>1:
...
def build_wrn_model(images, num_classes, wrn_size):
  ...
  with tf.variable_scope('init'):
    ...
    #
    #tf.logging.info('Sensing matrix generation for wrn')
    x = ops.fc_sensing_matrix(x,output_filters,300,scope='init_conv')
  ...
  for block_num in range(1, 4):
    ...
    for i in range(1, num_blocks_per_resnet):
      with tf.variable_scope('unit_{}_{}'.format(block_num, i)):
        activate_sigmoid = True if i == 2 else False
        activate_relu = True if i <= 3 else False
        x = residual_block(
            x,
            filters[block_num],
            filters[block_num],
            1,
            activate_before_residual=False,activate_sigmoid=activate_sigmoid,activate_relu=activate_relu,block_num=block_num)

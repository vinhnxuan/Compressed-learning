...
def fc_sensing_matrix(inputs,
       num_units_out,m_size,
       scope=None,
       reuse=None):
  """Creates a fully connected layer applied to `inputs`.
  ...
  tf.logging.info('Sensing matrix generation')
  if len(inputs.shape) > 2:
    channel_size = int(inputs.shape[3])
    a_size_1= int(inputs.shape[1])
    a_size_2= int(inputs.shape[2])
    n_size= a_size_1*a_size_2
    
    inputs = tf.transpose(inputs,perm=[0,3,1,2])
    inputs = tf.reshape(inputs, [-1, n_size])

  with tf.variable_scope(scope, 'FC_sensing_matrix', [inputs], reuse=reuse):
    weights_shape = [n_size, m_size]
    unif_init_range = 1.0 / (m_size)**(0.5)
    weights_initializer = tf.random_uniform_initializer(
        -unif_init_range, unif_init_range)
    weights = variable(
        name='weights_1',
    ...
    biases = variable(
        ...
        trainable=True)
    outputs = tf.nn.xw_plus_b(inputs, weights, biases)
  outputs = tf.reshape(outputs, [-1,channel_size, m_size])
  
  num_units_out_wide= num_units_out*n_size  
  with tf.variable_scope(scope, 'FC_sensing_matrix_inverse', [outputs], reuse=reuse):
    weights_shape = [channel_size, m_size, num_units_out_wide]
    unif_init_range = 1.0 / (num_units_out)**(0.5)
    weights_initializer = tf.random_uniform_initializer(
        -unif_init_range, unif_init_range)
    weights = variable(
    ...
    biases = variable(
        name='biases_2',
        shape=[num_units_out_wide,],
        dtype=tf.float32,
        initializer=bias_initializer,
        trainable=True)
    outputs = tf.tensordot(outputs, weights, axes=((1,2),(0,1)))+biases#tf.nn.xw_plus_b(inputs, weights, biases) 
  outputs = tf.reshape(outputs, [-1,a_size_1,a_size_2,num_units_out])
  return outputs

...
tf.flags.DEFINE_string('model_name', 'wrn',
                       ...
                       'pyramid_net')
tf.flags.DEFINE_string('checkpoint_dir', '/tmp/training/compressive/wrn', 'Training Directory.')
tf.flags.DEFINE_string('data_path', '/home/ads/gw438/Downloads/models-master/data_set/cifar_10/',
...
def build_model(inputs, num_classes, is_training, hparams):
  ...
  with contextlib.ExitStack() as stack:
    ...
    [stack.enter_context(arg) for arg in [*scopes]]
...
#      gvars= {var.op.name:var for var in tvars if not ('init_conv' in var.name)}
#      
#      self._saver = tf.train.Saver(gvars)
  ...
  def _compute_final_accuracies(self, meval):
    ...
    valid_accuracy = self.eval_child_model(meval, self.data_loader, 'test')
  ...
  def run_model(self):
    ...
    with tf.Graph().as_default(), tf.device(
        '/cpu:0' if FLAGS.use_cpu else '/gpu:2'):
      ...
      for curr_epoch in iter(range(starting_epoch, hparams.num_epochs)):
        ...
        if test_max==0 or test_max<valid_accuracy:
            test_max= valid_accuracy
...
def main(_):
  ...
  if FLAGS.model_name == 'wrn':
    ...
    hparams.add_hparam('num_epochs', 500)
